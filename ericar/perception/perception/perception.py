...
# --- S자 커브 끝 감지 ---
SC_WHT_MIN_RATIO  = 0.215  # ROI 흰색 픽셀 비율 하한
SC_GRN_MIN_RATIO  = 0.315  # ROI 초록 픽셀 비율 하한
SC_LIDAR_DIST_MIN = 15.0   # m — 라이다 탐색 시작 거리
SC_LIDAR_DIST_MAX = 20.0   # m — 라이다 탐색 끝 거리
SC_LIDAR_MIN_PTS  =    6   # 탐색 범위 내 포인트 수 하한
...
class Perception(Node):
    ...
    def _show_debug(self):
        ...
        try:
            ...
            if self._mode == MODE_S_CURVE:
                draw_s_curve_debug(
                    self._img_front, self._scan,
                    wht_min_ratio=SC_WHT_MIN_RATIO,
                    grn_min_ratio=SC_GRN_MIN_RATIO,
                    lidar_dist_min=SC_LIDAR_DIST_MIN,
                    lidar_dist_max=SC_LIDAR_DIST_MAX,
                    lidar_min_pts=SC_LIDAR_MIN_PTS,
                )
            cv2.waitKey(1)
        except Exception as e:
    ...
    def _detect_s_curve_end(self):
        ...
        result, wht, grn, lidar = detect_s_curve_end(
            self._img_front, self._scan,
            wht_min_ratio=SC_WHT_MIN_RATIO,
            grn_min_ratio=SC_GRN_MIN_RATIO,
            lidar_dist_min=SC_LIDAR_DIST_MIN,
